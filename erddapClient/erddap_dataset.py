...
class ERDDAP_Dataset:
    ...
    def __init__(self, erddapurl, datasetid, protocol='tabledap', auth=None, lazyload=True, request_kwargs={}):
        ...
        if self.erddapauth:
            self.request_kwargs.update({ 'auth' : self.erddapauth })
    ...
    def getData(self, filetype=DEFAULT_FILETYPE, request_kwargs={}):
        ...
        if request_kwargs:
            ...
            _request_kwargs = self.request_kwargs

        rawRequest = urlread(self.getDataRequestURL(filetype), **_request_kwargs)
